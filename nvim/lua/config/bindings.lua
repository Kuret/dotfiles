...
if vim.g.vscode then
  -- Vim-test --
  map('n', '<leader>tt', '<cmd>call VSCodeNotifyVisual(\'extension.elixirRunTestAtCursor\', 1)<CR>', {silent=true})
  map('n', '<leader>tf', '<cmd>call VSCodeNotifyVisual(\'extension.elixirRunTestFile\', 1)<CR>', {silent=true})
  map('n', '<leader>ta', '<cmd>call VSCodeNotifyVisual(\'extension.elixirRunTestSuite\', 1)<CR>', {silent=true})
  map('n', '<leader>tv', '<cmd>call VSCodeNotifyVisual(\'extension.elixirJumpToTest\', 1)<CR>', {silent=true})
  ...
  -- Copilot --
  map('i', '<C-j>', 'copilot#Accept("\\<CR>")', {silent=true, expr=true}) -- Copilot accept suggestion
  map('i', ']]', 'copilot#Next()', {silent=true, expr=true}) -- Copilot accept suggestion
  map('i', '[[', 'copilot#Previous()', {silent=true, expr=true}) -- Copilot accept suggestion
  ...
  -- WinShift  --
  map('n', '<leader>ws', ':WinShift<CR>', {silent=true})
end
